...
if __name__ == '__main__':
    ...
    save_path = '/mnt/122a7683-fa4b-45dd-9f13-b18cc4f4a187/homoglyphs/word_dump_centered_japan/'
    ...
    for i in tqdm(range(0,N)):
        ...
        seq_len = np.random.choice(range(1, 14))
        rand_size = np.random.choice(range(70, 133))

        seq_len = seq_len
        
        random_chars = np.random.choice(char_list, seq_len) # Make sure the char is in all the fonts
        random_chars_and_spaces = random_chars.tolist()
        np.random.shuffle(random_chars_and_spaces)
        ...
        for font_path_id in tqdm(range(0,len(font_paths))):# generate over different fonts, horizontal/vertical
            ...
            try:
                image_name_H, image_name_V, rimg_H, rimg_V = render_seg(font_paths, save_path, font_path_id, random_chars_and_spaces, rand_size) # The font paths are in, so we can iterate over all fonts. Can directly single out here, do not need the train, test, val
