...
if __name__ == '__main__':
    ...
    #Import G1 and pos_G1
    with open('G1.pickle', 'rb') as handle1:
        G1 = pickle.load(handle1)
    with open('pos_G1.pickle', 'rb') as handle2:
        pos_G1 = pickle.load(handle2)
    ...
    adjusted_node_size = dict([(node, 0.2*np.log(degree)) if degree < 16 else (node, 0.14*degree) for node, degree in networkx.degree(G1)])
    ...
    Avg_Top3_Zaibatsu_Distance_d = networkx.get_node_attributes(G1, 'Avg_Top3_Zaibatsu_Distance')
    #Replace the value of 999 with 15 (which is more easily for coloring)
    ...
    networkx.set_node_attributes(G1, Avg_Top3_Zaibatsu_Distance_d, 'Avg_Top3_Zaibatsu_Distance_d')
    ...
    Avg_Top3_Zaibatsu_Distance_d= networkx.get_node_attributes(G1, 'Avg_Top3_Zaibatsu_Distance_d')
    color_by_this_attribute = 'Avg_Top3_Zaibatsu_Distance_d'

    #Choose a title!
    title = 'Clean OCR Multimodal CLIPPINGS'
    ...
    graph = from_networkx(G1, pos_G1, scale=1, center=(0,0))
    ...
    print('Exporting Clean OCR_Multimodal CLIPPINGS.html.....')
