...
if __name__ == '__main__':
    ...
    path = os.path.join(os.getcwd(), '/path/to/noisyocr/df/gcv_2_gcvtk_lev_1_31861.csv')
    df = pd.read_csv(path)

    df=preprocess_df(df,'partner_main_title','partner_type','lev_gcv_2_gcvtk_matched_word_1')
    ...
    df_list['source'] = df.apply(lambda x: x['partner_main_title'] if x['partner_type'] == 'fanmai' else x['lev_gcv_2_gcvtk_matched_word_1'], axis=1)
    ...
    df_list['target'] = df.apply(lambda x: x['lev_gcv_2_gcvtk_matched_word_1'] if x['partner_type'] == 'fanmai' else x['partner_main_title'], axis=1)
    ...
    with open('G2.pickle', 'wb') as handle:
