...
def infer_hardneg(query_paths, ref_dataset, model, index_path, transform, inf_save_path,inf_check_save_path, k=8):
    ...
    with open(inf_check_save_path, 'w') as f:
        f.write("\n".join(all_nns_char))
...
if __name__ == "__main__":
    ...
    def main():
        ...
        # scheduler = CosineAnnealingWarmRestarts(optimizer, 20, 2)
        ...
        for epoch in range(args.start_epoch, args.num_epochs+args.start_epoch):
            ...
            trainer(enc, loss_func, device, train_loader, optimizer, epoch, args.epoch_viz_dir, args.diff_sizes, scheduler=None)
        ...
        ##Now evaulate on our paddle test set
        print("Evaluating on paddle test set...")

        # test_lang_images.main(root_folder="/mnt/122a7683-fa4b-45dd-9f13-b18cc4f4a187/PaddleOCR_testing/Paddle_test_images/zs_vit",
        # model=args.auto_model_timm,trained_model_path="/mnt/122a7683-fa4b-45dd-9f13-b18cc4f4a187/homoglyphs/" + args.run_name+ "/enc_best.pth",
        # lang_code="ja",wandb_log=True)
        


        # print("Infering hard negatives...")
        # if not args.infer_hardneg_k is None and args.N_classes is None:
        #     query_paths = glob.glob(args.random_font_path+"/*/*")
        #     print(f"Num hard neg paths: {len(query_paths)}")
        #     transform = BASE_TRANSFORM
        #     infer_hardneg(query_paths, render_dataset_single, best_enc, 
    ...
    # 'method': 'random',
    # 'name': 'sweep',
    # 'metric': {'goal': 'maximize', 'name': 'ja_accuracy_ocr'},
    # 'parameters': 
    # {
    #     'lr': {'max': 0.0003, 'min': 0.00002},
    #     'weight_decay': {'max': 0.2, 'min': 0.0004},
    #     'temp': {'max': 0.5, 'min': 0.0002},
    #     'm':{ 'values': [4,8,16]},
    #     'batch_size': {'values': [128,256]},
    #     'pretrained_model': {'values': ['vit_base_patch16_224','vit_base_patch16_224.dino']},
    #  }
    # }
